...
function BlockquoteWithImage({ author, image, children, className }) {
  return (
    <figure 
      className={clsx("grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-8 sm:grid-cols-12 sm:grid-rows-[1fr, auto,auto, 1fr] sm:gap-x-10 lg:gap-x-16",
      className
      )}
    >
  <blockquote className="col-span-2 text-xl/7 text-casal-light sm:col-span-7  sm:col-start-6 sm:row-start-2">
    {typeof children === "string"  ? <p>{children}</p> : children}
  </blockquote>
  <div className="col-start-1 row-start-2 overflow-hidden rounded-xl bg-casal-light sm:col-span-5 sm:row-span-full sm:rounded-3xl">
    <Image
     alt=""
     {...image}
  ...
  return(
    <Border position="left" className={clsx("pl-8", className)}>
      ...
      <blockquote className="text-neutral-600 [&>*]:relative [&>:first-child]:before:absolute [&>:first-child]:before:right-full [&>:first-child]:before:content-['“'] [&>:last-child]:after:content-['”']">
        ...
        <figcaption className="mt-6 font-semibold text-casal-dark">
